...
def mask_pii(data, pii_columns):
    """Mask PII columns - show first half, mask second half with asterisks"""
    if not pii_columns:
        return data
    ...
    for row in data:
        ...
        for pii_col in pii_columns:
            if pii_col in masked_row and masked_row[pii_col] is not None:
                ...
                half = len(value) // 2
                masked_row[pii_col] = value[:half] + value[half:].translate(str.maketrans(value[half:], '*' * len(value[half:])))
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        ...
        print("Example: python pii_masking.py s3://bucket/report.json s3://bucket/dedup.json s3://bucket/output/ /path/to/local/")
        sys.exit(1)
